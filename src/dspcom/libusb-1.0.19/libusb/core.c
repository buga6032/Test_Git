...
#ifdef YS_PRINT_API
int (*usbprint)(const char *fmt, ...) = printf;
#endif /* YS_PRINT_API */
	...
	libusb_device_handle **handle)
{
	...
	if (!dev->attached) {
#ifdef YS_PRINT_API
		YSPRINT("%s:%d %s: open %d.%d\n", __FILE__, __LINE__, __FUNCTION__, dev->bus_number, dev->device_address);
#endif
	...
	int interface_number)
{
	...
	if (!dev->dev->attached) {
#ifdef YS_PRINT_API
		YSPRINT("%s:%d %s: interface %d\n", __FILE__, __LINE__, __FUNCTION__, interface_number);
#endif
		return LIBUSB_ERROR_NO_DEVICE;
	}
	...
	unsigned char endpoint)
{
	...
	if (!dev->dev->attached) {
#ifdef YS_PRINT_API
		YSPRINT("%s:%d %s: endpoint %d\n", __FILE__, __LINE__, __FUNCTION__, endpoint);
#endif
		return LIBUSB_ERROR_NO_DEVICE;
	}
	...
	uint32_t num_streams, unsigned char *endpoints, int num_endpoints)
{
	...
	if (!dev->dev->attached) {
#ifdef YS_PRINT_API
		YSPRINT("%s:%d %s: streams %u eps %d\n", __FILE__, __LINE__, __FUNCTION__,(unsigned) num_streams, num_endpoints);
#endif
		return LIBUSB_ERROR_NO_DEVICE;
	}
	...
	unsigned char *endpoints, int num_endpoints)
{
	...
	if (!dev->dev->attached) {
#ifdef YS_PRINT_API
		YSPRINT("%s:%d %s: eps %d\n", __FILE__, __LINE__, __FUNCTION__, num_endpoints);
#endif
		return LIBUSB_ERROR_NO_DEVICE;
	}
...
#ifdef YS_PRINT_API
int API_EXPORTED libusb_set_print_funcp(int (*func)(const char *fmt, ...))
{
	if (!func)
		return -1;
