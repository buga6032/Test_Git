...
#ifdef YS_PRINT_API
	YSPRINT("%s:%d %s: URBs are discarded\n", __FILE__, __LINE__, __FUNCTION__);
#endif
...
static int submit_bulk_transfer(struct usbi_transfer *itransfer)
{
	...
	for (i = 0; i < num_urbs; i++) {
		...
		if (r < 0) {
			if (errno == ENODEV) {
#ifdef YS_PRINT_API
				YSPRINT("%s:%d %s: ioctl failed, retval %d\n", __FILE__, __LINE__, __FUNCTION__, r);
#endif
...
//		YSPRINT("%s:%d %s: EAGAIN\n", __FILE__, __LINE__, __FUNCTION__);
	...
	if (r < 0) {
		if (errno == ENODEV) {
#ifdef YS_PRINT_API
			YSPRINT("%s:%d %s: LIBUSB_ERROR_NO_DEVICE\n", __FILE__, __LINE__, __FUNCTION__);
#endif
			return LIBUSB_ERROR_NO_DEVICE;
		}
